setwd("C:/Users/Gordon/PycharmProjects/blanka_v1/tmp")
...
peaks1 <- as.data.frame(read.table("C:/Users/Gordon/PycharmProjects/blanka_v1/tmp/peaks1.csv", header=TRUE, col.names=c("mz")))
peaks1 <- as.data.frame(peaks1[order(peaks1$mz),])
colnames(peaks1) <- c("mz")

peaks2 <- as.data.frame(read.table("C:/Users/Gordon/PycharmProjects/blanka_v1/tmp/peaks2.csv", header=TRUE, col.names=c("mz")))
peaks2 <- as.data.frame(peaks2[order(peaks2$mz),])
colnames(peaks2) <- c("mz")

peaks3 <- as.data.frame(read.table("C:/Users/Gordon/PycharmProjects/blanka_v1/tmp/peaks3.csv", header=TRUE, col.names=c("mz")))
peaks3 <- as.data.frame(peaks3[order(peaks3$mz),])
colnames(peaks3) <- c("mz")

peaks4 <- as.data.frame(read.table("C:/Users/Gordon/PycharmProjects/blanka_v1/tmp/peaks4.csv", header=TRUE, col.names=c("mz")))
peaks4 <- as.data.frame(peaks4[order(peaks4$mz),])
colnames(peaks4) <- c("mz")

peak_list <- list(peaks1, peaks2, peaks3, peaks4)

consensus <- consensusPeakList(peak_list, tol=1, cutoff=0.7)

write.csv(consensus, file="consensus_peak_list.csv", row.names=FALSE)
